...
Поскольку радиус сх-ти рядов $\sum\limits_{n=0}^{\infty}a_{n}z^{n}$ и $\sum\limits_{n=0}^{\infty}na_{n}z^{n-1}$ одинаков (см. [[Лемма о радиусе сходимости#Сл-е|Lm]]), дост. д-ть дифф. для $m=1$.
...
\end{gathered}
$$
для $(*)$ нужно проверить равномерну. сх-ть (см. [[Перестановка пределов в функ последовательностях|th]])
$$
...
но $\sum\limits_{n=1}^{\infty}|a_{n}|nr^{n-1}$ — сх-ся $\implies$ [[Признак Вейерштрасса]].