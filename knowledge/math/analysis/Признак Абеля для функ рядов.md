...
Осталось д-ть, что $\sum\limits_{k=1}^{\infty} \alpha_{k}(b_{k}-b_{k+1})\rightrightarrows$. Проверим [[Критерий Коши для равномерной сходимости ряда|кр. Коши]]. Возьмём $\varepsilon >0$, $b_{n}$ — равном. огр. $\implies \lvert b_{n}(x) \rvert\leq M\ \forall x \in E\ \forall n$
