...
$$
1. по [[Лемма об оценке квадратичной формы|Lm]] $Q(h)\geq c\lVert h \rVert^{2} \implies f(a+h)-f(a)\geq c\lVert h \rVert^{2}+o(\lVert h \rVert^{2})=\lVert h \rVert^{2}(c+o(1))>0$ при мал. $h$
2. анал.
