...
tags: []
---
$f: D\to \mathbb{R}^{n},\ D\subset \mathbb{R}^{n}$ — откр., $f$ [[Непрерывная дифференцируемость функций многих переменных|непр.-дифф.]] в окр-ти $a \in D$, $A:=f'(a)$ — обратима.
...
$$
Подберём $R>0$, т.ч. при $y \in B_{R}(b): G_{y}(\overline{B}_{r}(a))\subset \overline{B}_{r}(a)$ (далее оценка по [[Теорема об оценке функции через производную]]) 
$$
\begin{gathered}
