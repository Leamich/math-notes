...
$u_{n} \in C(K),\ u_{n}\geq 0, S(x):= \sum\limits_{n=1}^{\infty}u_{n}(x)$ (см. [[knowledge/math/analysis/Пространство непрерывных функций|Пространство непрерывных функций]])

Если $S \in C(K)$, то [[knowledge/math/analysis/Функциональный ряд|ряд]] равном. сх-ся.
# Док-во

$S_{n}, r_{n} \in C(K)$ — част. суммы и [[knowledge/math/analysis/Остаток ряда|Остаток ряда]] соотв.
...
$r_{m}(x_{n_{k}})\to r_{m}(x_{*})$ ($r_{m} \in C(K)$) $\implies r_{m}(x_{*})\geq \varepsilon\ \forall m\implies r_{m}(x_{*})\not\to 0$ $\implies$ ряд расх-ся (см. [[knowledge/math/analysis/Теорема о сходимости функциональных рядов#^849f4e|необх. усл-е]]). *Пр-чие*.
