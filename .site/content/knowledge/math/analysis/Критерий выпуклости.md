...
$$
Применим на $[u;v]$ [[knowledge/math/analysis/Теорема Лагранжа (о конечных приращениях)|т. Лагранжа]]: $\frac{f(v)-f(u)}{v-u}=f'(\xi_{1})$. Аналогично для $[v;w]: \frac{f(w)-f(v)}{w-v}=f'(\xi_{2})$. 

$u<\xi_{1}<v<\xi_{2}<w\implies f'(\xi_{1})\leq f'(\xi_{2})$ (т.к. пр-я возр.).

Тогда по [[knowledge/math/analysis/Лемма о трех хордах|Lm]]: $f$ выпукла.
