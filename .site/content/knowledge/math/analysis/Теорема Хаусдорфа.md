...
Если $K$ [[knowledge/math/analysis/Вполне ограниченное множество|вполн. огр.]] и $(K, \rho)$ — [[knowledge/math/analysis/Полное метрическое пространство|полное]] $\implies K$ — компакт.
# Док-во

$K\subset \bigcup\limits_{\alpha \in I}U_{\alpha}$ 

Пусть нельзя выделить конечн. [[knowledge/math/analysis/Подпокрытие|подпокрытие]].

$\varepsilon=1$. Сущ-ет конечн. $1$-сеть $S_{1}\implies K\subset \bigcup\limits_{x \in S_{1}}\overline{B}_{1}(x)$. 
![[assets/images/Pasted image 20250331184226.png]]
...
Тогда $\exists U_{\alpha_{0}}:x_{*} \in U_{\alpha_{0}}$ $\implies \exists r>0: x_{*} \in B_{r}(x_{*})\subset U_{\alpha_{0}}$ (т.к. $U_{\alpha}$ — [[knowledge/math/analysis/Открытое множество|открытые]]).
...
\end{gathered}
$$
![[assets/images/Pasted image 20250331191014.png]]
