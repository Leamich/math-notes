---
title: "Перестановка суммы и предела"
tags:
  - понятие
---
...
$u_{n}: E\to \mathbb{R},\ a$ — пр. т. $E$, $\sum\limits_{n=1}^{\infty} u_{n}(x)$ равном. сх-ся и $\lim\limits_{ x \to a }u_{n}(x)=c_{n}$ (см. [[knowledge/math/analysis/Функциональный ряд|Функциональный ряд]])
...
\end{gathered}
$$
Тогда по [[knowledge/math/analysis/Перестановка пределов в функ последовательностях|th]] $\exists \lim\limits_{ n \to \infty }b_{n}$, т.е. ряд $\sum\limits_{n=1}^{\infty}c_n$ сх-ся.
$$
