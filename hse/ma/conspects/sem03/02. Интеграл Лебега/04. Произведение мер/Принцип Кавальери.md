...
$\nu$ — [[Полная мера]], $m:=\mu \times \nu,\ C\in \mathcal{A}\otimes \mathcal{B}$ (см. [[Произведение мер]])
...
Тогда $\tilde{m}E=\int\limits_{X} \sum\limits_{n=1}^{+\infty}\nu(E_{n})_{x}\,d\mu=\sum\limits_{n=1}^{+\infty}\int\limits_{X} \nu(E_{n})_{x}\,d\mu=\sum\limits_{n=1}^{+\infty}\tilde{m}(E_{n})$ (можем поменять сумму с инт., т.к. всё неотриц., см. [[Следствия теоремы Леви]])
